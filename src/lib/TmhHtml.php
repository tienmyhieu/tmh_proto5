...
//        $content .= $this->entity['id'];
    ...
    private function routeGroups(string $baseUrl, array $routeGroups): string
    {
        $content = '';
        $hasTwoTypes = 1 < count($routeGroups);
        $elementRoute = $hasTwoTypes ? $this->components['element_route2'] : $this->components['element_route1'];
        if ($hasTwoTypes) {
            ...
            if (count($routeGroups)) {
                $routeGroup = current($routeGroups);
                $routeGroup = $this->minimizeAttributes($routeGroup);
                $content .= str_replace('||title||', $routeGroup['title'], $this->components['element_title']);
                $content .= $this->routes($baseUrl, $elementRoute, $routeGroup['routes']);
            }
