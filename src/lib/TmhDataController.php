...
//        echo "</pre>";
    ...
    {
        return ['route_groups' => $this->getEntityRouteGroups()];
    }
    ...
    public function getEntityRouteGroups(): array
    {
        $primaryKey = $this->getEntityField('id');
        $tmpRouteGroups = [];
        if (in_array($primaryKey, array_keys($this->database['entity_route_group']))) {
            foreach ($this->database['entity_route_group'][$primaryKey]['route_groups'] as $entityRouteGroup) {
                $tmpRouteGroups[$entityRouteGroup] = $this->database['route_group'][$entityRouteGroup];
            }
        }
